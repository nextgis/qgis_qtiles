...
def mbtiles_connect(mbtiles_file, silent=True):
    try:
...
def optimize_database(cur, silent=True):
    if not silent:
...
def compression_do(cur, con, chunk, silent=True):
    if not silent:
...
def compression_finalize(cur, con, silent=True):
    if not silent:
