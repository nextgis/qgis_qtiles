...
from qtiles.core.compat import QGIS_3_42_2
...
class QgisLogger(logging.Logger):
    """Custom logger for QTiles plugin.
...
def load_logger() -> QgisLogger:
    ...
    is_debug_logs_enabled = QTilesSettings().is_debug_logs_enabled
...
def update_logging_level() -> None:
    """Update logging level based on QTiles settings."""
    is_debug_logs_enabled = QTilesSettings().is_debug_logs_enabled
    logger.setLevel(logging.DEBUG if is_debug_logs_enabled else logging.INFO)
