...
#
# This file is based on reference implementation from the PMTiles project:
...
def optimize_directories(entries, target_root_len):
    ...
    leaf_size = 4096
    while True:
        root_bytes, leaves_bytes, num_leaves = build_roots_leaves(
            entries, leaf_size
        )
...
class Writer:
    ...
    def write_tile(self, tileid, data):
        if (
            len(self.tile_entries) > 0
            and tileid < self.tile_entries[-1].tile_id
        ):
        ...
        if hsh in self.hash_to_offset:
            ...
            if (
                tileid == last.tile_id + last.run_length
                and last.offset == found
            ):
