    ...
    for zoomDir in getDirs(directory_path):
        ...
        for rowDir in getDirs(os.path.join(directory_path, zoomDir)):
            ...
            for current_file in os.listdir(
                os.path.join(directory_path, zoomDir, rowDir)
            ):
                file_name, ext = current_file.split(".", 1)
                f = open(
                    os.path.join(
                        directory_path, zoomDir, rowDir, current_file
                    ),
                    "rb",
                )
                file_content = f.read()
                ...
                if ext == image_format:
                    ...
                    if (count % 100) == 0:
                        ...
                        msg = "%s tiles inserted (%d tiles/sec)" % (
                            count,
...
def mbtiles_to_disk(mbtiles_file, directory_path, **kwargs):
    ...
    metadata = dict(
        con.execute("select name, value from metadata;").fetchall()
    )
    json.dump(
        metadata,
        open(os.path.join(directory_path, "metadata.json"), "w"),
        indent=4,
    )
    ...
    while t:
        ...
        if kwargs.get("scheme") == "wms":
            tile = os.path.join(
                tile_dir,
                "%03d.%s" % (int(y) % 1000, kwargs.get("format", "png")),
            )
        else:
            tile = os.path.join(
                tile_dir, "%s.%s" % (y, kwargs.get("format", "png"))
            )
        f = open(tile, "wb")
        f.write(t[3])
        f.close()
    ...
    msg = ""
    try:
        count = con.execute("select count(zoom_level) from grids;").fetchone()[
            0
        ]
        grids = con.execute(
